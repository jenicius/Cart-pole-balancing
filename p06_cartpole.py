...
def choose_action(state, mdp_data):
    ...
    action = np.dot(mdp_data['value'], mdp_data['transition_probs'][state])
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for i in range(num_states):
        if reward_counts[i, 1] == 0:
            continue
        mdp_data['reward'][i] = -reward_counts[i, 0]/reward_counts[i, 1]
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        action = np.dot(mdp_data['value'], transition_probs)
        mdp_data['value'] = reward + gamma*np.maximum(action[:, 0], action[:, 1])
        max_change = np.max(abs(mdp_data['value'] - old_value))
        itr_num += 1
